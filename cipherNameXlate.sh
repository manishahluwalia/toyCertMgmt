...
# Usage:
#   $0 -openssl <OPENSSL-CIPHERNAME>
# Prints out TLS RFC ciphername, or multiple names.
#
#   $0 -rfc <RFC-CIPHERNAME>
# Prints out openssl ciphername 
#
# Exit value:
#   0 - exactly 1 translation found
#   1 - Bad input
#   2 - No equivalent names (from rfc to openssl)
#   3 - Bad input ciphername. 
#   4 - >1 matches
#   5 - Unknown error

: ${1:?ERROR: Need to specify type of input ciphername}
: ${2:?ERROR: Need to specify input ciphername}


# The mapping file inlined below has the following format:
# OPENSSL-CIPHERNAME<TAB>RFC_CIPHERNAME
...
then
  ...
  echo "ERROR: Input ciphername can be either -rfc or -openssl" >&2
  exit 1
fi

matches=$(grep "$searchFor" <<END-OF-MAPPING | cut -d\	 -f $keep
...
EXP-KRB5-RC4-MD5	TLS_PSK_WITH_NULL_SHA
EXP-KRB5-RC4-MD5	TLS_DHE_PSK_WITH_NULL_SHA
EXP-KRB5-RC4-MD5	TLS_RSA_PSK_WITH_NULL_SHA
